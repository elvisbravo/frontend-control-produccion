...
function initDataTable() {
    ...
    document.addEventListener("click", function(e) {
        if (e.target.closest(".btn-editar-usuario")) {
            const usuarioId = e.target.closest(".btn-editar-usuario").getAttribute("data-usuario-id");
            editarUsuario(usuarioId);
        }
        if (e.target.closest(".btn-eliminar-usuario")) {
            const usuarioId = e.target.closest(".btn-eliminar-usuario").getAttribute("data-usuario-id");
...
function abrirModalAgregarUsuario() {
    ...
    const modal = new bootstrap.Modal(document.getElementById("modalAgregarEditarUsuario"));
    modal.show();
}
...
function buscarDocumento() {
    ...
    // TODO: Integrar con API de búsqueda de documentos (RENIEC, etc.)
    console.log("Buscando documento:", tipoDocumento, numeroDocumento);

    // Simulación de búsqueda
    if (typeof Swal !== "undefined") {
        Swal.fire({
            title: "Buscando...",
            text: `Verificando ${tipoDocumento}: ${numeroDocumento}`,
            icon: "info",
        ...
        setTimeout(() => {
            Swal.fire({
                ...
                text: "Los datos se han completado automáticamente",
                icon: "success"
            });
        }, 2000);
    } else {
        alert("Buscando " + tipoDocumento + ": " + numeroDocumento);
    }
}
...
function guardarUsuario() {
    ...
    const especialidadSelect = document.getElementById("especialidad");
    const especialidad = Array.from(especialidadSelect.selectedOptions).map(opt => opt.value);
    ...
    if (typeof Swal !== "undefined") {
        Swal.fire({
            ...
            const modal = bootstrap.Modal.getInstance(document.getElementById("modalAgregarEditarUsuario"));
            if (modal) modal.hide();
        });
    } else {
        alert(mensajeExito);
